...
// InsertVideo 插入视频相关信息
// 成功返回true
func InsertVideo(tx *sqlx.Tx, userId int64, playUrl string, coverUrl string, title string) bool {

	res, err := tx.Exec("insert into video(user_id, play_url, cover_url, title, publish_date) values(?,?,?,?,?)", userId, playUrl, coverUrl, title, time.Now())
	if err != nil {
	...
	if count == 0 {
		common.ErrLog("插入视频记录时RowsAffected为0", err.Error())
		return false
	}
	return true
}
...
func GetPublishedVideoListByUserId(userId int64) (bool, []Video) {
	var videos []Video
	err := common.Db.Select(&videos, "select * from video where user_id = ?", userId)
	if err != nil {
...
func SelectAuthorIdByVideoId(videoId int64) int64 {
	var authorId int64
	err := common.Db.Get(&authorId, "select user_id from video where id = ?", videoId)
	if err != nil {
...
func GetFavoredVideoListByUserId(userId int64) (bool, []Video) {
	var videos []Video
	err := common.Db.Select(&videos, "select * from video where id in (select video_id from video_favor where user_id = ? and deleted = 0)", userId)
	if err != nil {
...
func GetVideoList() (bool, []Video) {
	var videos []Video
	err := common.Db.Select(&videos, "select * from video order by publish_date desc limit 0, 30")
	if err != nil {
